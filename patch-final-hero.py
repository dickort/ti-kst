...
# Final TI hero finish: satin black body, blackout glass, TI lime wheels,
# smoother body normals, richer studio highlights, and no blinking FX.
...
# Add TI accent treatment for the actual imported wheel/caliper materials.
...
accent = needle + '''
          ...
          if (isTiCaliper) {
            material.color?.set(0xb8dc34);
            if ("metalness" in material) material.metalness = 0.42;
            if ("roughness" in material) material.roughness = 0.33;
            material.needsUpdate = true;
          }
'''
...
# Studio-light polish: broad white strips create controlled satin highlights,
# while the existing lime strip remains a subtle TI accent rather than a flash.
...
'''
light_replacement = '''  const stripLight = new THREE.RectAreaLight(0xd8ff3e, 2.4, 2.1, 0.42);
  stripLight.position.set(3.2, 1.25, -1.3);
  stripLight.lookAt(0, 0.8, 0);
  scene.add(stripLight);

  const studioTop = new THREE.RectAreaLight(0xffffff, 4.2, 5.4, 0.52);
  studioTop.position.set(0.3, 5.8, 1.4);
  studioTop.rotation.x = -Math.PI / 2.6;
  studioTop.lookAt(0, 0.82, 0.35);
  scene.add(studioTop);

  const studioSide = new THREE.RectAreaLight(0xdce6ff, 2.8, 4.2, 0.42);
  studioSide.position.set(-3.8, 2.9, 1.1);
  studioSide.lookAt(-0.2, 0.9, 0.2);
  scene.add(studioSide);

  const studioFront = new THREE.RectAreaLight(0xffffff, 1.9, 2.8, 0.32);
...
print('Final hero patch applied: satin black AMG, blackout glass, TI lime rims, smooth body, studio highlights, no blinking')
