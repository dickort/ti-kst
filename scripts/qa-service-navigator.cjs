...
(async()=>{
 ...
 const results=[];
 // JS API: argument is second, options is THIRD. Previously these timeouts
 // were inadvertently passed to the predicate and the 30s default remained.
 const ready=async page=>{await page.goto('http://127.0.0.1:4173/',{waitUntil:'networkidle',timeout:120000});await page.bringToFront();await page.waitForFunction(()=>window.__TI_NAV&&['true','error','no-webgl'].includes(document.querySelector('.hero').dataset.navReady),null,{timeout:120000,polling:100});assert.equal(await page.locator('.hero').getAttribute('data-nav-ready'),'true','Actual WebGL model must load');};
 ...
 for(const viewport of [{width:1440,height:900},{width:390,height:844}]){
  ...
  // Software-GPU motion checks keep all real geometry, textures and shaders.
  // Only raster resolution changes. Separate stills below use native DPR=1.
  ...
  assert(title.font>=(viewport.width>819?85:34),'Large hero title regressed');assert(title.scroll<=title.client+2,'Hero title overflows its container');
  await fit(page,'home');
  ...
  fs.writeFileSync(path.join(dir,`${viewport.width}-camera.json`),JSON.stringify({drawingBufferRatio:.125,samples,errors},null,2));
  await fit(page,'tuning');assert.equal(await page.locator('.nav-marker').count(),5);
  // Non-motion UI coverage uses the existing user-facing animation switch.
  await page.locator('.nav-motion').click();await page.locator('[data-nav-action="zone"][data-value="blackpack"]').click();await settle(page);
  ...
  await still.locator('[data-nav-action="home"]').first().click();await settle(still);assert.equal((await still.evaluate(()=>__TI_NAV.getState())).doorProgress,0);await still.close();
