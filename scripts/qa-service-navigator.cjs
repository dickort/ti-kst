...
(async()=>{
 ...
 const xvfb=spawn('Xvfb',[':98','-screen','0','1920x1080x24','-nolisten','tcp'],{stdio:'ignore'});
 process.on('exit',()=>xvfb.kill());await new Promise(r=>setTimeout(r,650));
 ...
 for(const viewport of [{width:1440,height:900},{width:390,height:844}]){
  ...
  await page.goto('http://127.0.0.1:4173/',{waitUntil:'networkidle',timeout:60000});await page.bringToFront();
  ...
  try{await page.waitForFunction(()=>__TI_NAV.getState().direction==='tune'&&!__TI_NAV.getState().moving,{timeout:30000,polling:100});}
  catch(error){
   const debug=await page.evaluate(()=>({hidden:document.hidden,visibility:document.visibilityState,focused:document.hasFocus(),scroll:scrollY,rect:document.querySelector('.hero').getBoundingClientRect().toJSON(),state:__TI_NAV.getState(),samples:window.navSamples}));
   fs.writeFileSync(path.join(dir,`${viewport.width}-failure.json`),JSON.stringify({debug,errors},null,2));console.log('STALLED',JSON.stringify({...debug,samples:debug.samples.slice(-5)}));await page.screenshot({path:path.join(dir,`${viewport.width}-failure.png`)});throw error;
  }
  const samples=await page.evaluate(()=>{window.recordNav=false;return window.navSamples;});
  const moving=samples.filter(s=>s.moving),unique=new Set(moving.map(s=>s.camera.map(v=>v.toFixed(4)).join(',')));
  fs.writeFileSync(path.join(dir,`${viewport.width}-camera.json`),JSON.stringify({samples,errors},null,2));
  assert(unique.size>=8,'Too few distinct intermediate camera frames: '+unique.size);
  const first=samples[0].camera,last=samples[samples.length-1].camera;assert(Math.hypot(...last.map((v,i)=>v-first[i]))>.5,'Camera did not travel');
  console.log('CAMERA_QA',JSON.stringify({viewport,mode,intermediateFrames:unique.size,errors}));
  assert.equal(errors.length,0,JSON.stringify(errors));assert.equal(await page.locator('.nav-marker').count(),5);
  await page.screenshot({path:path.join(dir,`${viewport.width}-tune.png`)});
  await page.locator('[data-nav-action="zone"][data-value="blackpack"]').click();await page.waitForFunction(()=>!__TI_NAV.getState().moving,{timeout:30000,polling:100});
  ...
  await page.locator('[data-nav-action="back"]').click();await page.locator('[data-nav-action="zone"][data-value="cabin"]').click();await page.waitForFunction(()=>!__TI_NAV.getState().moving,{timeout:30000,polling:100});
  ...
  await page.evaluate(()=>{__TI_NAV.selectDirection('preserve');__TI_NAV.selectDirection('restore');__TI_NAV.selectDirection('tune');});await page.waitForFunction(()=>!__TI_NAV.getState().moving,{timeout:30000,polling:100});const end=await page.evaluate(()=>__TI_NAV.getState());
  ...
  assert.equal(errors.length,0,JSON.stringify(errors));results.push({viewport,webgl:mode==='true',rig:inside.rigReady,intermediateFrames:unique.size,errors,state:end});await page.close();
 }
