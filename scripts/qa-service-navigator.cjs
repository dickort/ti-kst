...
(async()=>{
 ...
 for(const viewport of [{width:1440,height:900},{width:390,height:844}]){
  ...
  // Sample actual animation frames, not wall-clock sleeps on a software GPU.
  ...
  await page.waitForFunction(()=>__TI_NAV.getState().direction==='tune'&&!__TI_NAV.getState().moving,{timeout:30000});
  ...
  const first=samples[0].camera,last=samples[samples.length-1].camera;
  assert(Math.hypot(...last.map((v,i)=>v-first[i]))>.5,'Camera did not travel');
  ...
  await page.locator('[data-nav-action="zone"][data-value="blackpack"]').click();
  await page.waitForFunction(()=>!__TI_NAV.getState().moving,{timeout:30000});
  ...
  await page.locator('[data-nav-action="back"]').click();await page.locator('[data-nav-action="zone"][data-value="cabin"]').click();
  await page.waitForFunction(()=>!__TI_NAV.getState().moving,{timeout:30000});
  ...
  await page.waitForFunction(()=>!__TI_NAV.getState().moving,{timeout:30000});const end=await page.evaluate(()=>__TI_NAV.getState());
