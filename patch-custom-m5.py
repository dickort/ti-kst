...
replacement = '''  const loader = new GLTFLoader();
  const amgAsset = window.innerWidth < 820
    ? "./assets/mercedes_amg_gt63s_mobile.glb"
    : "./assets/mercedes_amg_gt63s_web.glb";

  const activateModel = (model, sourceLabel = "AMG") => {
    carModel = model;
    ...
    const scale = 5.18 / Math.max(longest, 0.001);
    ...
    loading.textContent = sourceLabel;
  ...
  loader.load(
    amgAsset,
    (gltf) => activateModel(gltf.scene, "Mercedes-AMG GT 63 S"),
    (event) => {
      if (event.total) {
    ...
    (error) => {
      console.warn("AMG GLB not available yet; using sculpted fallback.", error);
      try {
        activateModel(createM5F90Model(), "Fallback 3D");
      } catch (fallbackError) {
        console.warn("Fallback model could not build.", fallbackError);
        stage.classList.remove("premium-loading");
        canvas.remove();
        loading.remove();
        credit.remove();
...
new_anchors = '''  const anchorMap = {
    preserve: new THREE.Vector3(1.48, 1.16, -0.70),
    restore: new THREE.Vector3(-0.18, 1.52, 0.68),
    tune: new THREE.Vector3(-1.42, 0.58, 0.92)
  };'''
...
print('premium-car.js patched: real Mercedes-AMG GLB loader with mobile LOD and fallback')
