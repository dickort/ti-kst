...
/**
 * Original AMG driver-door rig, revision 2.
 * Requires amg_driver_cabin_desktop.glb or amg_driver_cabin_mobile.glb.
 * Do not run against the older, material-merged AMG assets.
 * Does not create another renderer or animation loop and does not modify the
 * production hero. Call update(performance.now()) from its existing loop.
 */
export function createDriverDoorController({ model, camera, controls = null, onStateChange = () => {}, requestRender = () => {} }) {
  ...
  if (!door || !cabin) throw new Error('Use the new object-preserving driverDoor/interiorCabin GLB; the old AMG asset has no door rig.');

  // +X is the driver side in THIS asset: verified from the actual steering
  // wheel in source object Mesh.299. Do not infer driver side from screen X.
  ...
  const look = controls?.target?.clone() || model.localToWorld(new THREE.Vector3(0, .45, .08));
  let viewMotion = null;
  let doorMotion = null;
  let progress = 0;
  let state = 'closed';
  let returnView = null;
  let dampingBefore = controls?.enableDamping;
  let disposed = false;
  ...
  function setState(next) {
    ...
    state = next;
    onStateChange({ state, doorProgress: progress });
  }

  function worldPoint(values) {
    model.updateWorldMatrix(true, false);
    return model.localToWorld(new THREE.Vector3(...values));
  }

  function captureView() {
    return { eye: camera.position.clone(), target: (controls?.target || look).clone() };
  }

  function stopCamera() {
    viewMotion = null;
    if (controls && dampingBefore !== undefined) controls.enableDamping = dampingBefore;
  }
  ...
  function moveCamera(destination, duration = 1850, delay = 0) {
    ...
    if (controls) {
      if (!viewMotion) dampingBefore = controls.enableDamping;
      controls.enableDamping = false;
    }
    viewMotion = { from, destination, a, b, start: performance.now() + delay, duration };
    requestRender();
  }
  ...
  function focus(zone = 'interior', { compact = false } = {}) {
    ...
    const preset = sourcePresets[zone];
    const target = worldPoint(preset.target);
    const eye = worldPoint(preset.eye);
    if (compact) eye.sub(target).multiplyScalar(1.2).add(target);
    moveCamera({ eye, target }, 1350);
  }

  function open({ compact = false } = {}) {
    if (disposed || state === 'open' || state === 'opening') return;
    if (state === 'closed') returnView = captureView();
    const preset = sourcePresets.interior;
    const target = worldPoint(preset.target);
    const eye = worldPoint(preset.eye);
    if (compact) eye.sub(target).multiplyScalar(1.2).add(target);
    // The real source cabin is already present before a gap can become visible.
    // No proxy chair, box dashboard, opposite-side cut, or missing interior.
    cabin.visible = true;
    doorMotion = { from: progress, to: 1, start: performance.now() + 650, duration: 1100 };
    moveCamera({ eye, target }, 1850);
    setState('opening');
  }
  ...
  function update(now = performance.now()) {
    ...
    if (doorMotion) {
      ...
      tmpQuaternion.setFromAxisAngle(axis, openAngle * progress);
      door.quaternion.copy(closeQuaternion).multiply(tmpQuaternion);
  ...
  const onManualStart = () => stopCamera();
  ...
  return {
    ...
    // Existing custom drag handlers may call this instead of OrbitControls.
    onManualInput: stopCamera,
    getState: () => ({ state, doorProgress: progress, moving: !!doorMotion || !!viewMotion }),
    dispose() {
      stopCamera();
      doorMotion = null;
      controls?.removeEventListener?.('start', onManualStart);
      door.quaternion.copy(closeQuaternion);
      disposed = true;
    }
  };
}
